...
import { policeid, url } from '../url';
import { useRoute } from '@react-navigation/native';

const PoliceSelectLocation = ({ navigation }) => {
  ...
  const selectid3 = route.params?.zid;
  const pid = route.params?.policeid;


    useEffect(() => {
        requestLocationPermission();
        fetchcoordinates();
        console.log(selectid3)
    }, []);
    ...
    const fetchcoordinates = async () => {
        try {
            const response = await fetch(`${url}/Admin/ViewASpecificZone?zid=${selectid3}`);
    ...
    const handleAddLocation = () => {
        if (markers.length > 0) {
            ...
            navigation.navigate('AddIncidentByPolice', { latitude, longitude,pid,selectid3});
        }
    };
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <Text style={{color:'black'}}>{selectid3}</Text>
            <Text style={{color:'black'}}>{pid}</Text>
...
export default PoliceSelectLocation;
